...
if (isset($_COOKIE['token'])) {
    try {
        ...
        if ($user->role === 'CLIENT' || ($user->role === 'ADMIN' && isset($_POST['CLIENT_ID']))) {
            ...
            $query = $bdd->prepare(
                "SELECT id_sender sender, id_receiver receiver, content, time, state
                FROM messages
            ...
            while($message = $query->fetch()) {
                ...
                if ($user->role === 'ADMIN') {
                    $message['sender'] = $message['sender'] === $user->id_utilisateur ? 'ADMIN' : 'CLIENT';
                    $message['receiver'] = $message['receiver'] === $user->id_utilisateur ? 'ADMIN' : 'CLIENT';
                }
                if ($user->role === 'CLIENT') {
                    $message['sender'] = $message['sender'] === $user->id_utilisateur ? 'CLIENT' : 'ADMIN';
                    $message['receiver'] = $message['receiver'] === $user->id_utilisateur ? 'CLIENT' : 'ADMIN';
                }
